...
{
    var xhttp = new XMLHttpRequest();
    xhttp.open("POST",url+"contract/contracts_user",false);
...
function load_contract_join()

{
    var xhttp = new XMLHttpRequest();
    xhttp.open("POST",url+"contract/contracts_user_join",false);
    xhttp.setRequestHeader("Content-Type","application/json");
    var obj =
    {
        id_join:user.id
...
function save_contract_inspect(id)
{
    localStorage.setItem("contract_id",id);
}
function draw_contracts(contracts)
{
     var contest_progress_div = $('#contest_active');
     var contest_finished_div = $('#contest_finished');


     console.log('size of contracts',contracts.length)
     for (let i=0;i<contracts.length;i++)
     {
        var status = contracts[i].status ; 
        var host_ = contracts[i].host ;
        var join = contracts[i].join;
        var contest=contracts[i].contest;
        var id_contest = contracts[i].id
        var price = contracts[i].price ;

        var $contract_ = `<div id="`+id_contest+`" class="gig-center-item completed-gig">
        <div class="gig-info">
        <a onclick="save_contract_inspect(`+id_contest+`)"href="contract_board.html" class="gig-title" id="contest_title">`+contest.name+`</a><br>
        <div class="items">
        <div class="item">Completed: <span class="highlight">`+contest.endedAt+`</span></div>
        
        <div  class="item">
        Amount: <span id="contract_price"class="highlight">`+price+`USD</span>
        </div>
        </div>
        </div>
        <div class="gig-info pull-right">
        <div id="work_with" class="item user-info">
        Employer: <a href="#">`+join.username+`</a>
        <div class="avatar">
        <img id="image_host" width="42" height="42" class="img-circle" src="`+host+join.profile_pic+`">
        </div>
        </div>
        </div>
        </div>`
        ...
        if (status=='progress')
        {
            contest_progress_div.append($contract_);
        }
        else
        {
            contest_finished_div.append($contract_);
