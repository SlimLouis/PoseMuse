...
function usr_clone(user)
{

    var string = ``+user[0].username.toUpperCase()+` from `+user[0].profile.country+` | PoseMuse` ;
    $('title').text(string);
  var $h1_user = `<h1>
  `+user[0].username+`        
  <small>(<ul class="comma-list">
          <li>`+user[0].profile.country+`</li>
          </ul>)</small>
          </h1>`
          $('#profile-content').prepend($h1_user);

        var country ="";
for ( let countz in countries)
{
   if (countries[countz].code==user[0].profile.country)
   {
       console.log(countries[countz].name);
       $('.location').text(countries[countz].name);
       country=countries[countz].name;
...
$('#country_').text(country);
$('#description').text(user[0].profile.description)
...
var img_url = host+user[0].profile_pic
